...
void NavPhysicsAgent::_bind_methods() {
	...
	ClassDB::bind_method(D_METHOD("_navphysics_done", "new_floor_position"), &NavPhysicsAgent::_navphysics_done);
...
void NavPhysicsAgent::_refresh_parameters() {
	...
	NavPhysicsServer::get_singleton()->body_set_params(data.body, friction_per_tick);
}
...
void NavPhysicsAgent::_navphysics_done(const Vector3 &p_floor_pos) {
	// keep a record of the move since the last position
	data.prev_move = data.floor_pos;
