...
#define GSIMD_CHECK_RANGE                  \
	if (!check_range(from, num, size())) { \
		return;                            \
	}
#define GSIMD_CHECK_BOTH_RANGE                   \
	if (!check_range(from, num, size())) {       \
		return;                                  \
	}                                            \
	if (!check_range(from2, num, arr2.size())) { \
		return;                                  \
	}
...
bool Simd_4i32::check_range(int from, int num, int size) const {
	...
	if ((unsigned int)from > (unsigned int)size)
		goto failed;
	if ((unsigned int)to > (unsigned int)size)
		goto failed;
	return true;
