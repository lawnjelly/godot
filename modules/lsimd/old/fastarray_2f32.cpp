...
#define IMPL_VALUE(A)                                                               \
	void FastArray_2f32::A(const Vector2 &val, int from, int num)                   \
			IMPL_PREPARE                                                            \
					m_Array.A(Quat(val.x, val.y, val.x, val.y), q_from, num_quads); \
	IMPL_RESTORE

#define IMPL_SIMPLE(A, B)                         \
	void FastArray_2f32::A(int from, int num)     \
			IMPL_PREPARE                          \
...
#define IMPL_DUAL(A)                                                                      \
	void FastArray_2f32::A(const Ref<FastArray_2f32> &arr2, int from, int from2, int num) \
			IMPL_PREPARE int q_from2;                                                     \
...
#define IMPL_RESULT(A, B)                                                                    \
	void FastArray_2f32::A(const Ref<FastArray_2f32> &arr2, int from, int from2, int num) {  \
		...
		float fTempBefore = 0.0f;                                                            \
		float fTempAfter = 0.0f;                                                             \
		int res_before = from - 1;                                                           \
		int res_after = from + num;                                                          \
		if (flags & RF_BEFORE) {                                                             \
			fTempBefore = m_Result[res_before];                                              \
		}                                                                                    \
		if (flags & RF_AFTER) {                                                              \
			fTempAfter = m_Result[res_after];                                                \
		}                                                                                    \
                                                                                             \
