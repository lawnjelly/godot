...
struct [[nodiscard]] FPoint2 {
	...
	bool is_equal_approx(const FPoint2 &p_v, f32 p_tolerance = Math::CMP_EPSILON) const { return Math::is_equal_approx(x, p_v.x, p_tolerance) && Math::is_equal_approx(y, p_v.y, p_tolerance); }
};
...
struct [[nodiscard]] FPoint3 {
	...
	bool is_equal_approx(const FPoint3 &p_v, f32 p_tolerance = Math::CMP_EPSILON) const { return Math::is_equal_approx(x, p_v.x, p_tolerance) && Math::is_equal_approx(y, p_v.y, p_tolerance) && Math::is_equal_approx(z, p_v.z, p_tolerance); }
};
