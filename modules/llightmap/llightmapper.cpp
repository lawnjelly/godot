...
void LightMapper::Backward_ProcessTexel(int tx, int ty)
{
//		if ((tx == 13) && (ty == 284))
//			print_line("testing");
	...
	for (int l=0; l<m_Lights.size(); l++)
	{
		Backward_ProcessTexel_Light(l, pos, normal, temp, nSamples);
		*pTexel += temp;
	}
...
// trace from the poly TO the light, not the other way round, to avoid precision errors
void LightMapper::Backward_ProcessTexel_Light(int light_id, const Vector3 &ptSource, const Vector3 &ptNormal, FColor &color, int nSamples)//, uint32_t tri_ignore)
{
	const LLight &light = m_Lights[light_id];
