...
void SoftRend::ScanConverter::get_x_min_max(int32_t p_y, int32_t &r_x_min, int32_t &r_x_max) const {
	...
	const int32_t *sb = &(scan_buffer.ptr())[p_y * 2];
	r_x_min = *sb++;
	r_x_max = *sb;
...
void SoftRend::ScanConverter::scan_convert_line(const Vector2 &p_min_y_vert, const Vector2 &p_max_y_vert, int32_t p_which_side) {
	...
	int32_t *p_scan = scan_buffer.ptr();
	p_scan += (j_start * 2) + p_which_side;

	for (int j = j_start; j < j_end; j++) {
		*p_scan = (int32_t)Math::ceil(curr_x);
		p_scan += 2;
		curr_x += x_step;
	}
}
...
void SoftRend::Node::build_tree_split() {
	...
	uint32_t w = (clip_rect.size.x + tile_length - 1) / tile_length;
	uint32_t h = (clip_rect.size.y + tile_length - 1) / tile_length;

	if ((w <= 1) && (h <= 1)) {
		DEV_ASSERT(clip_rect.size.x <= tile_length);
		DEV_ASSERT(clip_rect.size.y <= tile_length);
...
void SoftRend::flush() {
	...
	if (state.thread_cores > 1) {
		...
		for (uint32_t t = 0; t < _tiles.tiles.size(); t++) {
			thread_do_tile_work(t, frame_buffer_orig);
		}
	}
#else
	for (uint32_t t = 0; t < _tiles.tiles.size(); t++) {
...
void SoftRend::flush_final(uint32_t p_tile_id, uint32_t *p_frame_buffer_orig) {
	...
	for (uint32_t y = top; y < bottom; y++) {
		...
		const GData *gdata = tile.get_g(left, y);
		//const GData *gdata = &state.render_target->get_g(left, y);
...
uint32_t SoftRend::pixel_shader(uint32_t x, uint32_t y, const GData &g) const {
	// Background
	if (!g.tri_id_p1) {
