...
void SoftMesh::create_from_mesh(const RID &p_mesh) {
	rid = p_mesh;
...
void SoftMeshInstance::create_from_multimesh(VisualServerScene::Instance *p_instance) {
	RID multimesh_rid = p_instance->base;

	RID mesh_rid = VSG::storage->multimesh_get_mesh(multimesh_rid);
	if (!mesh_rid.is_valid()) {
		return;
	}
	_create_from_mesh(p_instance, mesh_rid);
}

void SoftMeshInstance::_create_from_mesh(VisualServerScene::Instance *p_instance, const RID &p_mesh_rid) {
	_mesh_id = g_soft_rend.meshes->find_or_create(p_mesh_rid);
...
void SoftMeshInstance::create_from_mesh(VisualServerScene::Instance *p_instance) {
