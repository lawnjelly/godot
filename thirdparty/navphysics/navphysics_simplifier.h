...
class Simplifier {
	struct Poly {
		const static int MAX_POINTS = NAV_PHYSICS_POLY_MAX_VERTS;
		bool active = true;
		u32 num_points = 0;
	...
	struct Data {
		...
		TVector<u32> final_inds;
		TVector<u32> final_num_poly_inds;
	...
	bool calc_poly_normal(Poly &p) const;
