...
#define NP_ERR_BAD_COMPARE(cond)                                         \
	if ((cond)) {                                                        \
		NP_ERR_PRINT("bad comparison function; sorting will be broken"); \
		break;                                                           \
	}
	...
	inline int partitioner(int p_first, int p_last, T p_pivot, T *p_array) const {
		...
		while (true) {
			while (compare(p_array[p_first], p_pivot)) {
				if (Validate) {
					NP_ERR_BAD_COMPARE(p_first == unmodified_last - 1);
				}
				p_first++;
			}
			p_last--;
			while (compare(p_pivot, p_array[p_last])) {
				if (Validate) {
					NP_ERR_BAD_COMPARE(p_last == unmodified_first);
				}
				p_last--;
			}
