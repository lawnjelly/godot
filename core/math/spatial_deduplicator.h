...
class SpatialDeduplicator {
	...
	public:
		...
		AABB calc_bound(const Span<Vector3> &p_verts);
...
public:
	...
	bool deduplicate_verts_only(const Span<uint32_t> &p_in_inds, const Span<Vector3> &p_in_verts, LocalVectori<Vector3> &r_out_verts, LocalVectori<uint32_t> &r_out_inds, real_t p_epsilon = 0.01);

	bool deduplicate_map(const Span<uint32_t> &p_in_inds, const Span<Vector3> &p_in_verts, LocalVectori<uint32_t> &r_vert_map, uint32_t &r_num_out_verts, LocalVectori<uint32_t> &r_out_inds, AABB *r_bound = nullptr, real_t p_epsilon = 0.01);

	void find_duplicate_positions(const Span<Vector3> &p_in_verts, LocalVectori<LinkedVerts> &r_linked_verts_list, real_t p_epsilon = 0.01);
};
