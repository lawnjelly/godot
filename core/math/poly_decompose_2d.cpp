...
void PolyDecompose2D::_debug_draw(const LocalVectori<Point> &p_edges, String p_filename) {
	...
	im.fill();
	Vector2 p_start = get_edge_pos(p_edges, 0).vec2();
	p_start.x = -p_start.x;

	im.l_move(p_start);
	im.l_draw_num(p_edges[0].pos_idx);
	for (int n = 0; n < p_edges.size(); n++) {
		Vector2 p = get_edge_pos(p_edges, n).vec2();
		p.x = -p.x;
...
bool PolyDecompose2D::decompose(const LocalVectori<Vec2i> &p_positions, List<LocalVectori<uint32_t>> &r_result) {
...
void PolyDecompose2D::sort_edgelist(LocalVectori<Point> &r_edges) {
	...
	for (int n = 0; n < r_edges.size(); n++) {
		const Vec2i pos = get_edge_pos(r_edges, n);
...
bool PolyDecompose2D::can_see(const LocalVectori<Point> &p_edges, int p_from, int p_to, bool p_change_is_positive) const {
	...
	const Vec2i &from_a = _positions[p_edges[p_from].pos_idx];
	const Vec2i &to_a = _positions[p_edges[p_to].pos_idx];
	...
	for (int i = range_from; i < range_to; i++) {
		...
		const Vec2i &from_b = _positions[p_edges[n].pos_idx];
...
real_t PolyDecompose2D::try_split_reflex_generic(const LocalVectori<Point> &p_edges, int p_reflex_id, int p_change, const Vec2i &p_reflex_edge_a, const Vec2i &p_reflex_edge_b, int &r_seg_start, int &r_seg_end) {
	...
	int num_edges = p_edges.size();
	const Vec2i &reflex_pos = get_edge_pos(p_edges, p_reflex_id);
	...
	for (int n = 1; n < num_edges; n++) {
		...
		const Vec2i &pos_before = get_edge_pos(p_edges, t - p_change);

		if (p_change > 0) {
			cross = pos_before.cross(pos, reflex_pos);
		} else {
			cross = pos.cross(pos_before, reflex_pos);
		}
...
void PolyDecompose2D::try_split_reflex(const LocalVectori<Point> &p_edges, int p_reflex_id, int &r_seg_start, int &r_seg_end, real_t &r_best_fit) {
...

	{
		...
		const Vec2i &reflex_pos = get_edge_pos(p_edges, p_reflex_id);
		const Vec2i &reflex_pos_next = get_edge_pos(p_edges, p_reflex_id + 1);
	...
	{
		// find the first edge of the reflex vert
		const Vec2i &reflex_pos_prev = get_edge_pos(p_edges, p_reflex_id - 1);
		const Vec2i &reflex_pos = get_edge_pos(p_edges, p_reflex_id);
